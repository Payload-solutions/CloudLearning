...
class LactobacillusRegression:
    ...
    def model_prediction(self, minimum_milk_proteins: float, titratable_acidity: float, pH_milk_sour: float, fat_milk_over_100mg_: float, target_data: float) -> str:
        ...
        pred_range = None
        if not os.path.exists("lact_model.json"):
            ...
            with open("lact_model.json", "r") as json_file:
                loaded_model_json = json_file.read()
    
            model_loaded = model_from_json(loaded_model_json)

            # Loading weigths
            model_loaded.load_weights("lact_model.h5")
    ...
    def defining_model_lact(self, input_data: int, learning_rate_val: float):
        ...
        if not os.path.exists("lact_model.json"):
            ...
            with open("lact_model.json", "w") as json_file:
                json_file.write(json_model)

            model.save_weights("lact_model.h5")
